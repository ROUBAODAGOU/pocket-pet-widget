...
PocketPal 是一个以 iPhone 主屏幕小组件为核心触点的本地虚拟宠物原型。Phase 1 的可复现 Xcode 工程与云端验证骨架、Phase 2 的宠物状态引擎与共享持久化已经完成，并已在 GitHub Actions 的 macOS runner 上通过真实编译与模拟器测试。

## 当前验证状态

- 仓库：<https://github.com/ROUBAODAGOU/pocket-pet-widget>
- Phase 2 成功运行：<https://github.com/ROUBAODAGOU/pocket-pet-widget/actions/runs/32822009290>
- Phase 2 验证提交：`9c6f5787a57f9364c8a2b1fb48cdb8a11e544e98`
- 结果：XcodeGen 工程生成成功；App 与 Widget Extension 编译成功；Unit Tests 24/24、UI Tests 1/1 通过，0 失败。
...
在 Xcode 中选择 `PocketPal` scheme 和任一 iOS 17+ iPhone Simulator，然后运行。当前 App 状态页会显示由 Phase 2 领域层生成的样例宠物快照；Phase 3 将加入领养、家园和 App 内互动页面。
